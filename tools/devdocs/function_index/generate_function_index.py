...
def enrich_with_urls(functions: List[Dict], repo_url: str = "https://github.com/sgl-project/sglang") -> List[Dict]:
...
def generate_minimal_format(functions: List[Dict]) -> List[str]:
    """Generate token-efficient minimal format."""
    lines = []
    current_file = None
    ...
    for func in sorted(functions, key=lambda x: (x['file'], x['line'])):
        ...
        # Ultra-compact: just name and basic params
        sig = func['signature']
        # Extract just function name and param names
        if '(' in sig:
            name_part = sig.split('(')[0].replace('async def ', '').replace('def ', '')
            params_part = sig.split('(')[1].rstrip(')')
            # Strip type annotations for minimal version
            params = []
            ...
            if func.get('class'):
                lines.append(f"  {func['class']}.{name_part}({', '.join(params)})")
            else:
                lines.append(f"{name_part}({', '.join(params)})")
                
    return lines


def generate_human_readable_format(functions: List[Dict], module_name: str) -> List[str]:
    ...
    for filepath in sorted(by_file.keys()):
        ...
        if module_funcs:
            ...
            for func in module_funcs:
                ...
                if func.get('doc_summary'):
                    lines.append(f"         📝 {func['doc_summary'][:70]}")
...
def compute_coverage_metrics(functions: List[Dict], root: Path) -> Dict:
    ...
    return {
        ...
        "documentation_rate": f"{(with_docs/len(functions)*100):.1f}%" if functions else "0%",
...
def main():
    parser = argparse.ArgumentParser(description="Generate function index for SGLang codebase")
    parser.add_argument('--root', type=Path, default=Path('python/sglang/srt'),
                       help='Root directory to scan')
    parser.add_argument('--out', type=Path, default=Path('docs/dev/refs/code-index'),
                       help='Output directory for artifacts')
    parser.add_argument('--format', choices=['detailed', 'minimal', 'both'], default='both',
                       help='Output format')
    parser.add_argument('--modules', type=str, default='',
                       help='Comma-separated module prefixes to limit scope')
    parser.add_argument('--per-directory', action='store_true',
    ...
    if args.format in ['minimal', 'both']:
        ...
        readable_lines = generate_human_readable_format(all_functions, "SGLang SRT Complete")
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"- **Documented**: {metrics['documented']} ({metrics['documentation_rate']})\n")
