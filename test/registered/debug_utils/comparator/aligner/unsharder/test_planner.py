...
class TestConcatedAxes:
    def test_concated_axis_skipped(self) -> None:
        """TP=4 + DP=2, dims='t(dp:concated) h(tp)' → only TP plan."""
        dim_specs = parse_dims("t(dp:concated) h(tp)")
        parallel_infos: list[dict[ParallelAxis, AxisInfo]] = []
    ...
    def test_concated_only_returns_empty(self) -> None:
        """Only concated axis → plans=[]."""
        dim_specs = parse_dims("t(dp:concated) h")
        parallel_infos: list[dict[ParallelAxis, AxisInfo]] = [
    ...
    def test_concated_not_treated_as_replicated(self) -> None:
        """DP=2 in parallel_info + t(dp:concated) → no PickParams for DP."""
        dim_specs = parse_dims("t(dp:concated) h(tp)")
        parallel_infos: list[dict[ParallelAxis, AxisInfo]] = [
            {
                ParallelAxis.DP: AxisInfo(axis_rank=0, axis_size=2),
        ...
        assert isinstance(plans[0].params, ConcatParams)
        pick_plans = [p for p in plans if isinstance(p.params, PickParams)]
        assert len(pick_plans) == 0

    def test_concated_not_in_parallel_info(self) -> None:
        """DP concated in dims but DP absent from parallel_info → silently skipped."""
        dim_specs = parse_dims("t(dp:concated) h(tp)")
        parallel_infos: list[dict[ParallelAxis, AxisInfo]] = [
            {ParallelAxis.TP: AxisInfo(axis_rank=0, axis_size=2)},
            {ParallelAxis.TP: AxisInfo(axis_rank=1, axis_size=2)},
