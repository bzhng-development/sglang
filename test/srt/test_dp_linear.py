...
def test():
    ...
    t1_tensor = torch.tensor(t1, dtype=torch.float).cuda()
    dist.all_reduce(t1_tensor, op=dist.ReduceOp.MAX)
    if dist.get_rank() == 0:
        print(f"Kernel1, e2e completed in {float(t1_tensor.item()):.2f} ms")
    # Warmup for kernel2
    for _ in range(2):
    ...
    t2_tensor = torch.tensor(t2, dtype=torch.float).cuda()
    dist.all_reduce(t2_tensor, op=dist.ReduceOp.MAX)
    if dist.get_rank() == 0:
        print(f"Kernel2, e2e completed in {float(t2_tensor.item()):.2f} ms")
