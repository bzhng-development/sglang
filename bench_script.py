...
START_STOP_SLEEP_SECONDS = 180

# Serve command for baseline branch
SERVE_COMMAND_BASE: List[str] = [
    "python",
    "-m",
...
# Serve command for new branch (with enable-symm-mem)
SERVE_COMMAND_NEW: List[str] = [
    "python",
    "-m",
    "sglang.launch_server",
    "--model-path",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--baseline-branch",
        help="Git branch to use for the baseline benchmark run. If omitted, uses current branch.",
    )
    parser.add_argument(
        "--candidate-branch",
        help="Git branch to checkout for the candidate (second) benchmark run. If omitted, you will be prompted.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--skip-second-run",
        action="store_true",
        help="Skip the second benchmark run entirely (ignores --candidate-branch).",
    )
    parser.add_argument(
...
def launch_server(
    log_path: Path, serve_command: List[str]
) -> Tuple[subprocess.Popen, Iterable[Path]]:
    env = os.environ.copy()
...
def run_benchmark_cycle(
    ...
    server_process, resources = launch_server(server_log, serve_command)
...
def main() -> None:
    ...
    if not candidate_branch and not args.skip_second_run:
        candidate_branch = prompt_for_branch()
    ...
    if args.baseline_branch:
        ...
        checkout_branch(args.baseline_branch)
    ...
    results[f"baseline_{baseline_tag}"] = run_benchmark_cycle(
        ...
        serve_command=SERVE_COMMAND_BASE,
    )
    ...
    if candidate_branch:
        ...
        try:
            checkout_branch(candidate_branch)
            ...
            results[f"candidate_{branch_tag}"] = run_benchmark_cycle(
                ...
                post_start_delay=args.post_start_delay,
                accuracy_concurrency=args.accuracy_concurrency,
                serve_command=SERVE_COMMAND_NEW,
            )
        finally:
            checkout_branch(original_branch)
    elif args.baseline_branch:
        # If we checked out baseline branch but no candidate, return to original
        checkout_branch(original_branch)
