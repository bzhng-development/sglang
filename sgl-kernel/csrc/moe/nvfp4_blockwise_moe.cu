...
// SM103-specific optimized grouped GEMM using "ultra" NVF4 schedules.
// Key differences from SM100:
// 1. Uses Sm103 arch tag to enable ultra schedules
// 2. Uses deeper K tile (768 vs 128) to better utilize SM103's higher FP4 throughput
// 3. Uses cute::tuple<Element, SF> pattern required by block-scaled tensor ops
// 4. Uses NoSmem epilogue schedule for direct-store grouped GEMM
// 5. Uses SM103-specific ultra mainloop schedule
template <typename OutType>
void run_fp4_blockwise_scaled_group_mm_sm103(
    torch::Tensor& output,
    const torch::Tensor& a,
    const torch::Tensor& b,
    const torch::Tensor& a_blockscale,
  ...
  using ElementType = cutlass::float_e2m1_t;
  using ElementSFType = cutlass::float_ue4m3_t;
  // Use tuple<Element, SF> pattern for block-scaled tensor ops (required for SM103 ultra schedules)
  ...
  // Architecture definitions - SM103 for ultra schedules
  ...
  using ClusterShape = Shape<_1, _1, _1>;
  struct MMA1SMConfig {
    // SM103 uses deeper K tile (768) for better FP4 throughput utilization
    using MmaTileShape = Shape<_128, _128, _768>;
    // SM103-specific ultra mainloop schedule for grouped GEMM
    using KernelSchedule = cutlass::gemm::KernelPtrArrayTmaWarpSpecialized1SmBlockScaledMxNvf4UltraVs16Sm103;
    // NoSmem epilogue schedule for direct-store grouped GEMM
...
void cutlass_fp4_group_mm(
  ...
  if (sm_version == 103) {
    // SM103 uses optimized "ultra" NVF4 schedules with deeper K tile (768)
    if (output.scalar_type() == torch::kBFloat16) {
      run_fp4_blockwise_scaled_group_mm_sm103<cutlass::bfloat16_t>(
          output,
          a,
          b,
          a_blockscale,
      ...
      run_fp4_blockwise_scaled_group_mm_sm103<cutlass::half_t>(
          output,
          a,
          b,
          a_blockscale,
    ...
    // SM100 uses standard NVF4 schedules
    if (output.scalar_type() == torch::kBFloat16) {
