...
void launch_tinygemm2(__nv_bfloat16* gA, __nv_bfloat16* gB, __nv_bfloat16* gC, __nv_bfloat16* bias,
  ...
  host::RuntimeCheck(res == CUDA_SUCCESS, "cuTensorMapEncodeTiled failed for weight_map with error code ", (int)res);
  ...
  res = cuTensorMapEncodeTiled(
      ...
      CUtensorMapL2promotion::CU_TENSOR_MAP_L2_PROMOTION_NONE,
      CUtensorMapFloatOOBfill::CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
  host::RuntimeCheck(res == CUDA_SUCCESS, "cuTensorMapEncodeTiled failed for activation_map with error code ", (int)res);
  ...
  if (use_pdl) {
    ...
    host::RuntimeCheck(status == cudaSuccess, "cudaFuncSetAttribute failed: ", cudaGetErrorString(status));
    ...
    host::RuntimeCheck(status == cudaSuccess, "cudaLaunchKernelEx failed: ", cudaGetErrorString(status));
  } else {
    ...
    host::RuntimeCheck(status == cudaSuccess, "tinygemm_kernel launch failed: ", cudaGetErrorString(status));
  }
}

void tinygemm2_op(TensorView input, TensorView weight, TensorView bias, TensorView output,
                  bool use_pdl) {
  cudaStream_t stream = host::LaunchKernel::resolve_device(input.device());
