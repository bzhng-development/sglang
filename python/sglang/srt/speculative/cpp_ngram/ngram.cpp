...
std::vector<std::pair<TrieNode*, int32_t>> Ngram::match(const std::vector<int32_t>& tokens, size_t batch_size) const {
  ...
  auto max_match_window_size = static_cast<int32_t>(std::min(tokens.size(), param_.max_match_window_size));
  ...
  for (int32_t token : tokens) {
    ...
    std::sort(match_next_states_.begin(), match_next_states_.end(), [](const auto& lhs, const auto& rhs) {
  ...
  for (const auto& state : match_states_) {
    if (state.second >= static_cast<int32_t>(min_match_window_size) && state.second <= max_match_window_size) {
      match_results_.emplace_back(state);
    }
  }
  std::sort(match_results_.begin(), match_results_.end(), [](const auto& lhs, const auto& rhs) {
