...
from sglang.srt.debug_utils.comparator.dp_bundle_comparator import (
    any_side_has_dp,
    compare_bundle_pair_tensor_type_dp,
...
_any_side_has_dp = any_side_has_dp
_extract_dp_size_from_meta = extract_dp_size_from_meta
_group_by_dp_rank = group_by_dp_rank
...
def _compare_bundle_pair_inner(
    *,
    ...
    # 3. All values are tensors → route to DP or non-DP path
    if any_side_has_dp(all_pair):
        return compare_bundle_pair_tensor_type_dp(
            name=name,
            valid_pair=all_pair,
            token_aligner_plan=token_aligner_plan,
            diff_threshold=diff_threshold,
