...
from sglang.srt.debug_utils.comparator.aligner.token_aligner.dp_grouping import (
    build_rank_to_dp_rank,
    load_multi_dp_groups,
)
...
# re-export for existing callers and backward compat
__all__ = ["AUX_NAMES", "has_aux_tensors", "load_and_normalize_aux"]

# re-export for backward compat (tests import these)
_extract_dp_rank_from_meta = extract_dp_rank_from_meta_with_plugin
_build_rank_to_dp_rank = build_rank_to_dp_rank
...
def load_and_normalize_aux(
    ...
    rank_to_dp_rank: dict[int, int] = build_rank_to_dp_rank(
    ...
    return load_multi_dp_groups(
        dump_path=dump_path,
        df=df,
        plugin=plugin,
        dp_ranks=dp_ranks,
        rank_to_dp_rank=rank_to_dp_rank,
        load_single_dp_group_fn=_load_single_dp_group,
    )
