...
class FlashinferDispatcher(BaseDispatcher):
    ...
    def __init__(
        ...
        # Write MoE output directly into FI combine workspace when backend supports
        # an explicit output tensor.
        self.payload_in_workspace = (
            get_moe_runner_backend().is_flashinfer_cutlass()
            or (get_moe_runner_backend().is_flashinfer_trtllm())
        )
        # CUTLASS expects swizzled NVFP4 scales, TRTLLM routed FP4 path expects
        # linear scales.
        self.swizzle_nvfp4_scales = get_moe_runner_backend().is_flashinfer_cutlass()
