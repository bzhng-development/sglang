...
void sm100_fp8_pertensor_dispatch_shape(
  ...
  using Gemm256 = JitGemmFp8RowwiseC3x<
      ...
      EpilogueTileAuto,
      WithBias>;
  // Mid-size prefill bucket (256 < M <= 2048): deeper K tile than the default
  // to cut mainloop trips on large-K projections (K 5k..28k at M ~1-2k, where
  // the shallow-K default measurably trails cuBLASLt on B300).
  using Gemm2048 = JitGemmFp8RowwiseC3x<
      ArchTag,
      OutType,
  ...
  if (mp2 <= 2048) {
    return launch_c3x_fp8_rowwise_scaled_mm<Gemm2048>(out, a, b, scales_a, scales_b, bias, stream);
  }
